...
const NodeCanvas = ({
  ...
  return (
    <div style={{ position: 'relative' }}>
      {/* QR image shown top-right; it displays client/public/qr.png generated by prestart */}
      <div style={{
        position: 'absolute',
        top: 8,
        right: 8,
        ...
        <div style={{ fontSize: 12, marginBottom: 4 }}>Scan to open image</div>
        <img
          src="/qr.png"
          alt="QR code"
          style={{ width: 140, height: 140, objectFit: 'contain', display: qrVisible ? 'block' : 'none' }}
          onLoad={() => setQrVisible(true)}
          onError={() => setQrVisible(false)}
        />
        {!qrVisible && (
          <div style={{ fontSize: 11, color: '#555', textAlign: 'center', width: 140 }}>
            QR not available
          </div>
        )}
      </div>
